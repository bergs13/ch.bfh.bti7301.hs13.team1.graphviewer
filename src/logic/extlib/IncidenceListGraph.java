...
/**
 * A implementation of the Graph interface based on incidence lists (at each
 * vertex a list of all the edges incident to this vertex are are stored).
 * 
 * @author ps
 * 
 * @param <V>
 *            the type of the elements stored at the vertices of this graph
 * @param <E>
 *            the type of the elements stored at the edges of this graph
 */
public class IncidenceListGraph<V, E> extends IGLDecorable implements
		Graph<V, E> {
	...
	public Iterator<Edge<E>> incidentEdges(Vertex<V> v) {
		...
		final Iterator<Entry<ILGVertex, ILGEdge>> it = w.iEdges.entrySet()
				.iterator();
		return new Iterator<Edge<E>>() {
			@Override
			public boolean hasNext() {
	...
	public Iterator<Edge<E>> incidentInEdges(Vertex<V> v) {
		...
		if (w.thisGraph != this)
			throw new RuntimeException("Invalid Vertex!");
		if (!isDirected)
			throw new RuntimeException("undirected graph!");
		final Iterator<Entry<ILGVertex, ILGEdge>> it = w.inIEdges.entrySet()
				.iterator();
		return new Iterator<Edge<E>>() {
			@Override
			public boolean hasNext() {
	...
	public Iterator<Edge<E>> incidentOutEdges(Vertex<V> v) {
		...
		if (!isDirected)
			throw new RuntimeException("undirected graph!");
		final Iterator<Entry<ILGVertex, ILGEdge>> it = w.outIEdges.entrySet()
				.iterator();
		return new Iterator<Edge<E>>() {
			@Override
			public boolean hasNext() {
	...
	public boolean areAdjacent(Vertex<V> v1, Vertex<V> v2) {
		...
		if (w1.thisGraph != this)
			throw new RuntimeException("Invalid Vertex!");
		ILGVertex w2 = (ILGVertex) v2;
		if (w2.thisGraph != this)
			throw new RuntimeException("Invalid Vertex!");
	...
	public Edge<E> insertEdge(Vertex<V> from, Vertex<V> to, E elem) {
		...
		if (fromV.thisGraph != this)
			throw new RuntimeException("Invalid Vertex!");
		ILGVertex toV = (ILGVertex) to;
		if (toV.thisGraph != this)
			throw new RuntimeException("Invalid Vertex!");
	...
	public String toString() {
		...
		while (eit.hasNext()) {
			ILGEdge ev = eit.next();
			sb.append(ev.from.toString() + con + ev.to.toString() + "  "
					+ ev.toString() + "\n");
		}
		return sb.toString();
	}
	...
	public Vertex<V> opposite(Edge<E> e, Vertex<V> v) {
		...
		if (iEdge.from == w)
			...
			throw new RuntimeException(w + " is not an endpoint of " + iEdge);
	}
	...
	private class ILGEdge extends IGLDecorable implements Edge<E> {
		...
		ILGEdge(E e, ILGVertex from, ILGVertex to) {
			...
			this.to = to;
			if (isDirected) {
				if (from.outIEdges.containsKey(to))
					throw new RuntimeException("Parallel edges not allowed!");
				from.outIEdges.put(to, this);
				to.inIEdges.put(from, this);
			}
			if (!isDirected && from.iEdges.containsKey(to))
				throw new RuntimeException("Parallel edges not allowed!");
